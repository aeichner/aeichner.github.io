		...
		for case in switch(node.name):
			if case("element"):
				if node.prop("ref") is not None:
					...
					return XMLFsm.particle(self.createContentModel(ref, stack), minOccurs, maxOccurs)
				...
				if node.prop("abstract") == "true":
					if self.substs.has_key(qname):
						...
						if len(content) > 0:
							return XMLFsm.particle(XMLFsm.choice(content), minOccurs, maxOccurs)
					return XMLFsm.empty()
			...
			if case("extension", "restriction"):
				...
				if content is None:
					...
					fsm = XMLFsm.sequence([baseContent, content])
				return fsm
				break

			if case("any"):
				return XMLFsm.particle(XMLFsm.element("*", XMLFsm.empty()), minOccurs, maxOccurs)
				break
